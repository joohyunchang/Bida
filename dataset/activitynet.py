...
class ActivityNetDataset(Dataset):
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
            frame_sample_rate=2, crop_size=224, short_side_size=256, new_height=256,
        ...
        self.dataset_samples = list(cleaned['video_id'])
        self.durations = list(cleaned['video_duration'])
        self.label_array = list(cleaned['label'])
        self.audio_samples = {cleaned.iloc[i, 0]: cleaned.iloc[i, 1:4] for i in range(len(cleaned))}
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            start_ratio = round(float(self.audio_samples[self.dataset_samples[index]]['t_start']) / float(self.audio_samples[self.dataset_samples[index]]['video_duration']),5)
            end_ratio = round(float(self.audio_samples[self.dataset_samples[index]]['t_end']) / float(self.audio_samples[self.dataset_samples[index]]['video_duration']),5)
            end_ratio = min(end_ratio, 1.0)
        
            buffer, all_idx = self.loadvideo_decord(sample, start_ratio, end_ratio, sample_rate_scale=scale_t, return_index=True) # T H W C
            ...
            if len(buffer) == 0:
                while len(buffer) == 0:
                        ...
                        end_ratio = round(float(self.audio_samples[self.dataset_samples[index]]['t_end']) / float(self.audio_samples[self.dataset_samples[index]]['video_duration']),5)
                        end_ratio = min(end_ratio, 1.0)
                        buffer, all_idx = self.loadvideo_decord(sample, start_ratio, end_ratio, sample_rate_scale=scale_t, return_index=True)
            ...
            buffer, all_idx = self.loadvideo_decord(sample, start_ratio, end_ratio, return_index=True)
            ...
            sample = os.path.join(self.data_path, sample)
            chunk_nb, split_nb = self.test_seg[index]
            start_ratio = round(float(self.audio_samples[self.test_dataset[index]]['t_start']) / float(self.audio_samples[self.test_dataset[index]]['video_duration']),5)
            end_ratio = round(float(self.audio_samples[self.test_dataset[index]]['t_end']) / float(self.audio_samples[self.test_dataset[index]]['video_duration']),5)
            end_ratio = min(end_ratio, 1.0)
    ...
    def loadvideo_decord(self, sample, start_ratio, end_ratio, sample_rate_scale=1, return_index=False, sampling_type='uniform'):
        ...
        if self.mode == 'test':
            ...
            if video_length <= 0:
                video_length = 1
            all_index = []
            tick = video_length / float(self.num_segment)
            all_index = list(np.array([start_frame + int(tick / 2.0 + tick * x) for x in range(self.num_segment)] +
                                [start_frame + int(tick * x) for x in range(self.num_segment)]))
        ...
        video_length = end_frame - start_frame
        if video_length <= 0:
            # print(f"Warning: video_length is zero or negative. Adjusting end_frame. {sample}")
            video_length = 1
        ...
        if sampling_type == 'uniform':
            ...
            num_segment, clip_len = self.clip_len, self.num_segment
            converted_len = int(clip_len * self.frame_sample_rate)
            ...
            if all_index: # all_index가 비어있지 않을 때만 연산 수행
                all_index = list(np.array(all_index) + start_frame)
