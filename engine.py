...
def train_one_epoch(args, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if data_iter_step%50==0:
            ave=np.mean(ave_latency[3:])
            final_latency.append(ave)
            print('latency is {:.2f}'.format(ave))
            ave_latency=[]
        if data_iter_step==1000:
            print(f'final_ave_step_time is {np.mean(final_latency[1:])}')
            exit(0)
        ...
        # fwd_throughput= samples.shape[0]/(curr_time)
        latency=curr_time*1000
        ave_latency.append(latency)
