...
class CrossAttentionS2Audio(nn.Module):
    def __init__(self, dim: int, audio_dim: int, n_head: int, num_frames: int, spec_frames: int, attn_all_frame = False, 
                 audio_patch = 196, attn_mask: torch.Tensor = None, time_encoding=False, spec_shape=None):
        super().__init__()
...
class B_CAST(nn.Module):
    def __init__(self, dim, num_heads, num_frames=16, down_ratio=2, audio_dim=512, audio_num_heads=8, 
                 ...
                 attn_all_frame=True, spec_frames=8, audio_patch=196, skip_connect=True, time_encoding=False, spec_shape=None):
        super().__init__()
        self.skip_connect = skip_connect
        self.num_frames = num_frames
        ...
        if type == 's-text':
            text_dim, text_num_heads = audio_dim, audio_num_heads
            self.cross_r_down = nn.Linear(text_dim, text_dim//self.down_ratio)
            self.ln_r_cross = norm_layer(text_dim//self.down_ratio)
            self.l2r_cross = CrossAttentionS2Text(dim//self.down_ratio, text_dim//self.down_ratio, text_num_heads, num_frames, attn_all_frame)
            self.r2l_cross = CrossAttentionText2S(dim//self.down_ratio, text_dim//self.down_ratio, text_num_heads, num_frames, attn_all_frame)
            self.cross_r_up = nn.Linear(text_dim//self.down_ratio, text_dim)
        elif type == 't-text':
            text_dim, text_num_heads = audio_dim, audio_num_heads
            self.cross_r_down = nn.Linear(text_dim, text_dim//self.down_ratio)
            self.ln_r_cross = norm_layer(text_dim//self.down_ratio)
            self.l2r_cross = CrossAttentionT2Text(dim//self.down_ratio, text_dim//self.down_ratio, text_num_heads, num_frames, attn_all_frame)
            self.r2l_cross = CrossAttentionText2T(dim//self.down_ratio, text_dim//self.down_ratio, text_num_heads, num_frames, attn_all_frame)
            self.cross_r_up = nn.Linear(text_dim//self.down_ratio, text_dim)
        elif type == 's-t':
            ...
            self.l2r_cross = CrossAttentionS2Audio(dim//self.down_ratio, audio_dim//self.down_ratio, audio_num_heads, num_frames, spec_frames, attn_all_frame, audio_patch, time_encoding=time_encoding, spec_shape=spec_shape)
            self.r2l_cross = CrossAttentionAudio2S(dim//self.down_ratio, audio_dim//self.down_ratio, audio_num_heads, num_frames, spec_frames, attn_all_frame, audio_patch, time_encoding=time_encoding, spec_shape=spec_shape)
            self.cross_r_up = nn.Linear(audio_dim//self.down_ratio, audio_dim)
        elif type == 't-audio':
            self.cross_r_down = nn.Linear(audio_dim, audio_dim//self.down_ratio)
            self.ln_r_cross = norm_layer(audio_dim//self.down_ratio)
            self.l2r_cross = CrossAttentionT2Audio(dim//self.down_ratio, audio_dim//self.down_ratio, audio_num_heads, num_frames, spec_frames, attn_all_frame, audio_patch, time_encoding=time_encoding, spec_shape=spec_shape)
            self.r2l_cross = CrossAttentionAudio2T(dim//self.down_ratio, audio_dim//self.down_ratio, audio_num_heads, num_frames, spec_frames, attn_all_frame, audio_patch, time_encoding=time_encoding, spec_shape=spec_shape)
            self.cross_r_up = nn.Linear(audio_dim//self.down_ratio, audio_dim)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, num_frames=16, mlp_ratio=4., down_ratio=2, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                ...
                time_encoding=False, spec_shape=None):
        super().__init__()
        ...
        if num_layer >= late_fusion:
            ...
            if self.num_layer in self.CA:
                if audio_enabled:
                    self.s_text_b_cast = B_CAST(dim, num_heads, num_frames, down_ratio, text_dim, text_num_heads, drop_path, act_layer, norm_layer, type='s-audio', spec_frames=spec_frames, attn_all_frame=attn_all_frame, audio_patch=audio_patch, skip_connect=skip_connect, time_encoding=time_encoding, spec_shape=spec_shape)
                    self.t_text_b_cast = B_CAST(dim, num_heads, num_frames, down_ratio, text_dim, text_num_heads, drop_path, act_layer, norm_layer, type='t-audio', spec_frames=spec_frames, attn_all_frame=attn_all_frame, audio_patch=audio_patch, skip_connect=skip_connect, time_encoding=time_encoding, spec_shape=spec_shape)
                else:
    ...
    def forward(self,s_x, t_x, text, time_encodings=None, output_attentions=None):
        B = t_x.shape[0]
        n, bt, _ = s_x.shape
        num_frames = bt//B
        ...
        if self.num_layer >= self.late_fusion:
            if self.bcast_method == 'seq':
                ...
                if self.num_layer in self.CA:
                    if self.audio_enabled:
                        s_x, text = self.s_text_b_cast(s_x, text, time_encodings, output_attentions=output_attentions)
                        t_x, text = self.t_text_b_cast(t_x, text, time_encodings, output_attentions=output_attentions)
                    else:
                ...
                if self.num_layer in self.CA:
                    if self.audio_enabled:
                        s_x_ct, text_ct = self.s_text_b_cast(s_x, text, time_encodings, output_attentions=output_attentions)
                        t_x_vt, text_vt = self.t_text_b_cast(t_x, text, time_encodings, output_attentions=output_attentions)
                    else:
                        s_x_ct, text_ct = self.s_text_b_cast(s_x, text)
                        t_x_vt, text_vt = self.t_text_b_cast(t_x, text)
                else:
...
class STCrossTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        if not audio_enabled:
            ...
            self.clip_text_positional_embedding = nn.Parameter(text_scale * torch.randn(audio_patch+ 1, text_dim))
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                time_encoding=self.time_encoding, spec_shape=spec_shape)
    ...
    def forward_features(self, x, spec=None, caption=None, split_projection=False, time_encodings=None, output_attentions=None):
        B = x.shape[0]
        ...
        if self.pre_time_encoding:
            text_x[2:,:,:] = text_x[2:,:,:] + rearrange(time_encodings[0], 'b t n d -> n (b t) d')
        ...
        for blk in self.blocks:
            ...
            s_x, t_x, text_x = blk(s_x, t_x, text_x, time_encodings, output_attentions=output_attentions)
        s_x = s_x.permute(1,0,2)
    ...
    def forward(self, x, spec=None, caption=None, idx=None, output_attentions=None):
        ...
        if self.composition:
            if not self.split_projection or self.audio_enabled:
                if self.audio_enabled:
                    s_x, t_x, text_x = self.forward_features(x, spec=spec, time_encodings=time_encodings, output_attentions=output_attentions)
                else:
...
@register_model
def cast_Bsquare_CA9_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=512, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, CA=9, output_text_dim=512,
        prefix = 16, postfix = 16, **kwargs)
    return model

@register_model
def cast_Bsquare_CA9_down4_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=512, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, CA=9, output_text_dim=512,
        prefix = 16, postfix = 16, down_ratio=4, **kwargs)
    return model

@register_model
def cast_Bsquare_CA0_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=512, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, CA=0, output_text_dim=512, 
...
@register_model
def cast_Bsquare_CA9_lavila_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=512, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, CA=9, output_text_dim=256, 
        prefix = 16, postfix = 16, **kwargs)
    return model

@register_model
def cast_Bsquare_CA0_lavila_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=512, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, CA=0, output_text_dim=256,
        prefix = 16, postfix = 16, **kwargs)
    return model
...
@register_model
def cast_audio_Bsquare_CA9_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        ...
        prefix = 16, postfix = 16, attn_all_frame=False, **kwargs)
    return model

@register_model
def cast_audio_Bsquare_CA0_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
...
@register_model
def cast_single_audio_Bsquare_ALLCA9_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, audio_enabled=True, text_num_heads=12, CA=9, output_text_dim=768,
...
@register_model
def cast_stacks_audio_Bsquare_CA9_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, audio_enabled=True, text_num_heads=12, CA=9, output_text_dim=768,
...
@register_model
def cast_stacks_audio_Bsquare_CA9_down4_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, audio_enabled=True, text_num_heads=12, CA=9, output_text_dim=768,
        prefix = 16, postfix = 16, spec_frames=16, attn_all_frame=False, down_ratio=4, **kwargs)
    return model
...
def cast_single_audio_Bsquare_CA0_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, audio_enabled=True, text_num_heads=12, CA=0, output_text_dim=768,
        prefix = 16, postfix = 16, spec_frames=1, attn_all_frame=True, **kwargs)
    return model
...
@register_model
def compo_cast_single_audio_Bsquare_CA9_down4_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, audio_enabled=True, text_num_heads=12, CA=9, output_text_dim=768,
        prefix = 16, postfix = 16, spec_frames=1, attn_all_frame=True, down_ratio=4, **kwargs)
...
@register_model
def compo_cast_single_audio_Bsquare_CA0_down4_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, audio_enabled=True, text_num_heads=12, CA=0, output_text_dim=768,
        prefix = 16, postfix = 16, spec_frames=1, attn_all_frame=True, down_ratio=4, use_Adapter=True, **kwargs)
    return model

@register_model
def compo_cast_single_audio_Bsquare_ALLCA9_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, audio_enabled=True, text_num_heads=12, CA=9, output_text_dim=768,
...
@register_model
def compo_cast_single_audio_Bsquare_ALLCA11_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, audio_enabled=True, text_num_heads=12, CA=11, output_text_dim=768,
        prefix = 16, postfix = 16, spec_frames=1, attn_all_frame=True, down_ratio=2, CA_eq=True, **kwargs)
    return model

@register_model
def compo_cast_single_audio_Bsquare_CA11_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, audio_enabled=True, text_num_heads=12, CA=11, output_text_dim=768,
        prefix = 16, postfix = 16, spec_frames=1, attn_all_frame=True, down_ratio=2, CA_eq=False, **kwargs)
    return model

@register_model
def compo_cast_single_audio_Bsquare_CA9_down8_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, audio_enabled=True, text_num_heads=12, CA=9, output_text_dim=768,
...
@register_model
def compo_cast_single_audio_Bsquare_CA0_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, audio_enabled=True, text_num_heads=12, CA=0, output_text_dim=768,
        prefix = 16, postfix = 16, spec_frames=1, attn_all_frame=True, **kwargs)
