...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, num_frames=16, mlp_ratio=4., down_ratio=2, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.ln_s_cross = nn.Identity()
        # self.ln_t_cross = nn.Identity()
        ...
        # self.t2s_cross = nn.Identity()
        # self.s2t_cross = nn.Identity()
    ...
    def forward(self,s_x, t_x):
        ...
        # c_s_x = self.cross_s_up(self.act(self.t2s_cross(n_s_x)))
        # c_t_x = self.cross_t_up(self.act(self.s2t_cross(n_t_x)))
...
@register_model
def bidir_wl_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, down_ratio=1, **kwargs)
    return model

@register_model
def compo_bidir_wl_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, down_ratio=1, **kwargs)
    return model