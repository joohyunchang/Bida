...
class CrossAttentionS2T(nn.Module):
    def __init__(self, dim: int, n_head: int, num_frames: int, spec_frames=1, attn_all_frame = True, 
        ...
        if not attn_all_frame:
            ...
            self.vmae_temporal_pos = nn.Parameter(self.scale * torch.randn((num_frames//2, dim))) if not audio_only else nn.Parameter(self.scale * torch.randn((spec_frames, dim)))
        else:
            ...
            if self.use_stpos:
                self.clip_space_pos = nn.Parameter(self.scale * torch.randn((audio_patch, dim)))
                self.vmae_space_pos = nn.Parameter(self.scale * torch.randn((video_patch, dim))) if not audio_only else nn.Parameter(self.scale * torch.randn((audio_patch, dim)))
                self.clip_temporal_pos = nn.Parameter(self.scale * torch.randn((spec_frames, dim)))
                self.vmae_temporal_pos = nn.Parameter(self.scale * torch.randn((num_frames//2, dim))) if not audio_only else nn.Parameter(self.scale * torch.randn((spec_frames, dim)))
        if self.time_embedding_type:
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, num_frames=16, mlp_ratio=4., down_ratio=2, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 relative_position_embedding=True, num_buckets=320, max_distance=800, gru_rel_pos=True, late_fusion=0, use_SA=True, use_MLP=True, time_encoding=False, spec_shape=None, video_patch=196, clip_only=False, time_embedding_type=False, use_stpos=True):
        super().__init__()
    ...
    def forward(self, s_x, t_x, pos_bias=None, time_encodings=None, output_attentions=None):
...
class STCrossTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                audio_patch=audio_patch, use_AIM=use_AIM, audio_only=audio_only, relative_position_embedding=self.relative_position_embedding, num_buckets=self.num_buckets, max_distance=self.max_distance, gru_rel_pos=gru_rel_pos,
    ...
    def forward_features(self, x, spec=None, time_encodings=None, output_attentions=None):
        ...
        if not self.audio_only:
            ...
            t_x = t_x + self.clip_text_positional_embedding.to(t_x.dtype) # bt hw+1 d
        ...
        for blk in self.blocks:
            layer_outputs = blk(s_x, t_x, pos_bias, time_encodings, output_attentions=output_attentions)
            s_x, t_x, pos_bias = layer_outputs[0], layer_outputs[1], layer_outputs[2]
        t_x = t_x.permute(1,0,2)
...
@register_model
def single_beats_only_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, audio_enabled=True, 
        CA=0, spec_frames=1, attn_all_frame=True, use_AIM=False, audio_only=True, **kwargs)
    return model
...
def compo_single_beats_clip_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        ...
        CA=0, spec_frames=1, attn_all_frame=True, **kwargs)
    return model
...
@register_model
def compo_single_beats_only_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, audio_enabled=True, 
        CA=0, spec_frames=1, attn_all_frame=True, use_AIM=False, audio_only=True, **kwargs)
