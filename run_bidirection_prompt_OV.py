...
def get_args():
    ...
    parser.add_argument('--data_set', default='Kinetics-400', choices=['EPIC_dense','diving-48','Kinetics-400', 'SSV2','MINI_SSV2', 'UCF101', 'HMDB51','image_folder', 'EPIC','EPIC_OV'],
    ...
    parser.add_argument('--audio_type', default='all8', choices=['all','all8','frame','stack','stacks','single','onespec','single1024','stackss','single1024s'],
                        type=str, help='audio_trim_type')
    parser.add_argument('--time_encoding', action='store_true', default=False)
    parser.add_argument('--openvocabulary', action='store_true', default=False)
...
def main(args, ds_init):
    ...
    if args.kd:
        if args.text_finetune is not None:
            class_list = text_prompt(dataset=args.data_set, data_path=args.anno_path, clipbackbone=args.clip_finetune, device=args.device, text_finetune=args.text_finetune, useEncoder=True)
        else:
        ...
        if args.text_finetune is not None:
            class_list = text_prompt(dataset=args.data_set, data_path=args.anno_path, clipbackbone=args.clip_finetune, device=args.device, text_finetune=args.text_finetune)
        else:
    ...
    if args.composition:
        ...
        # else:
        from engine_for_prompt_OV import train_one_epoch, validation_one_epoch, final_test, merge
    else:
        if args.kd:
