...
class STCrossTransformer(nn.Module):
    ...
    def replace_text_embedding(self, actionlist, actiondict, actiontoken, videofeature = None, embedding = 'noun'):
        if videofeature is not None:
            ...
            if embedding == 'noun':
                text_embedding = (self.noun_embedding(torch.arange(77).to(self.device))[None, :].repeat([B,1,1]) + videofeature.unsqueeze(1).repeat(1, 77, 1)).unsqueeze(1).repeat([1 ,len(actionlist), 1, 1])
            else:
                text_embedding = (self.verb_embedding(torch.arange(77).to(self.device))[None, :].repeat([B,1,1]) + videofeature.unsqueeze(1).repeat(1, 77, 1)).unsqueeze(1).repeat([1, len(actionlist), 1, 1])
