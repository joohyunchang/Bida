...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, num_frames=16, mlp_ratio=4., down_ratio=2, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., init_values=None, num_layer=0, act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn_head_dim=None,use_aim=False):
        super().__init__()
    ...
    def forward(self,s_x, t_x):
        ...
        if self.use_aim:
            # AIM Space MHSA
            xt = rearrange(s_x, 'n (b t) d -> t (b n) d', t=num_frames)
        ...
        s_x = s_x + self.AS_Adapter(self.attention(self.clip_ln_1(s_x))) # original space multi head self attention
    ...
    def __init__(self, 
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                init_values=init_values, num_layer=i,use_aim=self.use_aim)
...
@register_model
def bidir_aim_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, use_aim=True, **kwargs)
    return model

@register_model
def compo_bidir_aim_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=True, use_aim=True, **kwargs)
    return model