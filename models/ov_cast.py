...
class STCrossTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.composition:
            ...
            self.verb_embedding = torch.nn.Embedding(context_length, self.text_dim)
        else:
            self.noun_embedding = torch.nn.Embedding(context_length, self.text_dim)
        ...
        if self.composition:
            ...
            self.meta_V_Adapter = Adapter(text_dim, mlp_ratio=0.0625, skip_connect=False)
            # self.clip_ov_verb_proj = nn.Parameter(scale * torch.randn(embed_dim, proj_dim))
    ...
    def forward(self, x, inp_nounlist, inp_verblist=None):        
        if self.composition:
            ...
            if ov == 'both' or ov == 'verb':
                verb_embedding, prompt_verbtoken = self.replace_text_embedding(inp_verblist, self.verbdict, self.verbtoken, V_x, embedding= 'verb')
            else:
            ...
            B = s_x.shape[0]
            if noun_embedding.dim() == 4:
                noun_embedding = rearrange(noun_embedding, 'b c t d -> (b c) t d')
                prompt_nountoken = rearrange(prompt_nountoken, 'b c t -> (b c) t')
                nounFeature = self.encode_text(noun_embedding, prompt_nountoken)
                nounFeature = rearrange(nounFeature, '(b c) d -> b c d', b=B)
            else:
                nounFeature = self.encode_text(noun_embedding, prompt_nountoken)
                
            if verb_embedding.dim() == 4:
                verb_embedding = rearrange(verb_embedding, 'b c t d -> (b c) t d')
                prompt_verbtoken = rearrange(prompt_verbtoken, 'b c t -> (b c) t')
                verbFeature = self.encode_text(verb_embedding, prompt_verbtoken)
                verbFeature = rearrange(verbFeature, '(b c) d -> b c d', b=B)
            else:
                verbFeature = self.encode_text(verb_embedding, prompt_verbtoken)
...
@register_model
def ov_cast_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
...
@register_model
def ov_cast_down4_base_patch16_224(pretrained=False, args=None, class_list=None, **kwargs):
    textlist, textdict, texttoken = class_list
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, text_dim=512, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, 
