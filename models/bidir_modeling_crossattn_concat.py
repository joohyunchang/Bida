...
class STCrossTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        if self.composition:
            ...
            self.concat_head = nn.Linear(embed_dim * 2, 397)
            self.concat_head_dropout = nn.Dropout(head_drop_rate)
        else:
        ...
        if self.composition:
            self.concat_head.weight.data.mul_(init_scale)
            self.concat_head.bias.data.mul_(init_scale)
        else:
...
@register_model
def bidir_concat_vit_base_patch16_224(pretrained=False, **kwargs):
...
@register_model
def compo_bidir_concat_vit_base_patch16_224(pretrained=False, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
